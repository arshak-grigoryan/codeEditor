...
export default class View {
			...
			if (e.keyCode === 13) {
				...
				if (className === 'folderIcon' || !this.isProjectNow) {
					...
					expandArrow.addEventListener('click', (e) =>
						this.expandContent(e.target, ul),
					);

					if (!this.isProjectNow) {
						const iconsWrapper = createIconsWrapper();

						const fileIcon = iconsWrapper.children[0].addEventListener(
							'click',
							(e) => this.fileIconClick(e),
						);

						const folderIcon = iconsWrapper.children[1].addEventListener(
							'click',
							(e) => this.folderIconClick(e),
						);

						const editIcon = iconsWrapper.children[2].addEventListener(
							'click',
							(e) => this.editIconClick(e),
						);

						const deleteIcon = iconsWrapper.children[3].addEventListener(
							'click',
							(e) => this.deleteIconClick(e),
						);
	...
	editIconClick = (e) => {
		...
		if (
			...
			this.lastClickedFileOrFolder.getAttribute('class') === 'folder'
		) {
			const thisElemName = this.lastClickedFileOrFolder.children[0].children[0]
				.children[2].textContent;
			this.lastClickedFileOrFolder.children[0].children[0].children[2].remove();
			const inputName = createInput(
				this.lastClickedFileOrFolder.children[0].children[0],
				'editDiv',
				'editInput',
			);
			...
			inputName.addEventListener('keydown', (e) => {
				if (e.keyCode === 13) {
					...
					const span = createEl({
						tag: 'span',
						parentEl: this.lastClickedFileOrFolder.children[0].children[0],
						content: name,
					});
					inputName.parentElement.parentElement.remove();
				}
		...
		if (
			this.lastClickedFileOrFolder &&
			this.lastClickedFileOrFolder.getAttribute('class') === 'file'
		) {
			...
			inputName.addEventListener('keydown', (e) => {
				if (e.keyCode === 13) {
					...
					const iconimg = createEl({
						tag: 'img',
						parentEl: this.lastClickedFileOrFolder,
						attributes: { src: getIcon(name) },
					});
					const span = createEl({
						tag: 'span',
						parentEl: this.lastClickedFileOrFolder,
	...
	fileClick = (e) => {
		// e can be event or this li(file in explorer) element
		let name;
		let id;
		if (e instanceof MouseEvent) {
			...
			if (!this.isFileExcist(id)) {
				...
				file.children[2].addEventListener('click', (e) =>
					this.closeItem(e.target),
				);
				this.autoOpen(file);
			} else {
				const filesList = document.querySelectorAll('.listFile');
	...
	autoOpen = (e) => {
		// e can be event or this li(file in tabs menu) element
		if (e instanceof MouseEvent) {
			if (e.target.classList.contains('petqiVrov')) {
				return;
			}
	...
	folderClick = (e) => {
		...
		if (className === 'folder' || className === 'file') {
			this.lastClickedFileOrFolder = bindEl;
		}
	...
	deleteIconClick = (e) => {
		...
		if (
			this.lastClickedFileOrFolder === projectFolder ||
			this.lastClickedFileOrFolder === undefined
		) {
		...
		if (
			currentThis !== this.lastClickedFileOrFolder &&
			className === 'folder' &&
			!this.lastClickedFileOrFolder.classList.contains('file')
		) {
	...
	autoExpandContent = () => {
		...
		if (projectFolder === thisFolder) {
			projectFolder.children[0].children[0].children[0].style.transform =
				'rotate(0deg)';
			projectFolder.children[1].style.display = 'block';
			projectFolder.children[0].children[0].children[1].src =
				ICONS.folderOpenIcon;
		} else {
			thisFolder.parentElement.children[0].children[0].children[0].style.transform =
				'rotate(0deg)';
			thisFolder.parentElement.children[1].style.display = 'block';
			thisFolder.parentElement.children[0].children[0].children[1].src =
				ICONS.folderOpenIcon;
		}
	};
