...
export function createProjectButton() {
	...
	const createFolderWrapper = createEl({
		tag: 'div',
		classes: ['createFolderWrapper'],
		parentEl: root,
	});
	const text = createEl({
		tag: 'div',
		classes: ['createFolderText'],
		parentEl: createFolderWrapper,
	});
	createEl({
		tag: 'span',
		parentEl: text,
		content: 'You have not yet created folder.',
	});
	const createFolder = createEl({
		tag: 'div',
...
export function createIconsWrapper() {
	...
	const fileIcon = createEl({
		tag: 'img',
		classes: ['fileIcon'],
		parentEl: iconsWrapper,
		attributes: { src: ICONS.fileIconLightIcon },
	});
	const folderIcon = createEl({
		tag: 'img',
		classes: ['folderIcon'],
		parentEl: iconsWrapper,
		attributes: { src: ICONS.folderIconLightIcon },
	});
	const editIcon = createEl({
		tag: 'img',
		classes: ['editIcon'],
		parentEl: iconsWrapper,
		attributes: { src: ICONS.editIcon },
	});
	const deletion = createEl({
		tag: 'img',
		classes: ['deleteIcon'],
		parentEl: iconsWrapper,
		attributes: { src: ICONS.trashIcon },
	});
	return iconsWrapper;
}
...
export function creatFolder([name, parentElem]) {
	...
	const iconArrowWrapper = createEl({
		tag: 'div',
	...
	const iconArrow = createEl({
		tag: 'img',
		classes: ['iconArrow'],
		parentEl: iconArrowWrapper,
		attributes: { src: ICONS.expandArrowIcon },
	});
	if (parentElem) {
		createEl({
			tag: 'img',
			parentEl: iconArrowWrapper,
			attributes: { src: ICONS.folderOpenIcon },
		});
	}
	const span = createEl({
		tag: 'span',
...
export function createFile([name, parentElem, id]) {
	const li = createEl({
		...
		classes: ['file'],
		parentEl: parentElem,
		attributes: { tabindex: '0', 'data-id': id },
	});
...
export function createInput(parentElement, divClassName, InputClassName) {
	const root = document.getElementById('root');
	const inputNameDiv = createEl({
		tag: 'div',
		parentEl: parentElement || root,
		classes: ['inputNameDiv', divClassName],
	});
	const form = createEl({
		tag: 'form',
		parentEl: inputNameDiv,
	});
	const inputName = createEl({
		tag: 'input',
		parentEl: form,
		classes: ['inputNameInput', InputClassName],
...
export function findParent(el) {
	...
	if (attrClass === 'folder') {
		if (el.parentElement.getAttribute('id') === 'root') {
			return {
				currentThis: el,
				className: attrClass,
				name: el.children[0].children[0].children[1].textContent,
				bindEl: el,
			};
		}
		return {
			currentThis: el.children[1],
			className: attrClass,
			name: el.children[0].children[0].children[2].textContent,
			bindEl: el,
		};
	}
	if (attrClass === 'file') {
		if (el.parentElement.getAttribute('class') === 'folder') {
			return {
				currentThis: el.parentElement.children[1],
				className: attrClass,
	...
	if (attrClass === 'fileIcon' || attrClass === 'folderIcon') {
		return {
			currentThis: el.parentElement.parentElement.parentElement.children[1],
			className: attrClass,
		};
	}
...
export function createListItem(name, id) {
	...
	const close = createEl({
		...
		parentEl: li,
		classes: ['petqiVrov'],
		attributes: { src: ICONS.closeIcon },
	});
	return li;
}
